...
cd(get_absolute_file_path("wina.sce"));
f=read('wine.dat', 178, 14);
for i=1:1:59
    X(i,:)=[f(i,2:14) 1 0 0];
end
for i=60:1:130
...
v2=2;
g1=gca();
xlabel(g1, 'zawartość alkoholu');
ylabel(g1, 'zawartość kwasu jabłkowego');
tc=size(X, 'r');
for i=1:1:tc
    if T(i,1)>T(i,2)
        if T(i,1)>T(i,3)
